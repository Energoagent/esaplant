		...
		req.onreadystatechange = function() {
			if(this.readyState === 4 && this.status === 200) {
				...
				if (mill) {
					...
					if (params.BLK_CHAIN_INSTALL[0] === 1) {mill.fenceControl.alarm()} else {mill.chainControl.alarm()};
					if (params.FENCE_REM[0] === 1) {mill.fenceControl.alarm()} else {mill.fenceControl.alarm()};
				};
			};
		};
		req.send();
